...
function sanitizeProfile(input: unknown): UserHairProfile {
  ...
  return { face_shape: text("face_shape"), hair_type: text("hair_type"), hair_texture: text("hair_texture"), density: text("density"), length: text("length"), maintenance_level: text("maintenance_level") };
}
...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({
      data,
      profile,
      engine: { version: "1.3", confidence: recommendationConfidence(profile, data), weights: { face_shape: 35, hair_type: 20, hair_texture: 15, density: 10, length: 10, maintenance_level: 10 } },
    });
  } catch (error) {
