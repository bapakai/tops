...
const collection = [
  "Two Block", "Low Fade", "Textured Crop", "Comma Hair", "Middle Part", "French Crop",
  "Crew Cut", "Ivy League", "Buzz Cut", "Short Quiff", "Side Part", "Slick Back",
  "Pompadour", "Curtain Hair", "Wolf Cut", "Mullet", "Curly Top", "Spiky Hair",
  "Undercut", "Taper Fade", "Drop Fade", "Crop Fade", "Comb Over", "Messy Hair"
];
    ...
    <main className="site">
      ...
      {step === "home" && (
        <section className="home">
          <div>
            ...
            <h1>
              Gaya yang cocok,
              <em> sebelum dipotong.</em>
            </h1>
            <p>
              Rekam wajahmu sebentar. TOPSID membantu menemukan gaya rambut
          ...
          <div className="poster">
            ...
            <b>COLLECTION</b>
            <div>
              {collection.slice(0, 12).map((name, index) => (
                <article key={name}>
                  <span>{["◉", "◒", "◐", "◓"][index % 4]}</span>
                  <small>{name}</small>
                </article>
      ...
      {step === "camera" && (
        <section className="screen camera-screen">
          ...
          <p className="sub">
            Posisikan wajah di dalam oval, lalu hadap depan, sedikit ke kiri dan kanan.
          </p>
          ...
          <div className="controls">
            ...
            <button
              ...
              aria-label="Mulai atau hentikan rekaman"
            >
              <i />
            </button>
            <small>↻</small>
          ...
          {!cameraOn && (
            <label className="gallery">
              ...
              <input
                type="file"
                accept="image/*"
                onChange={(event) => handleGallery(event.target.files?.[0])}
              />
            </label>
          )}
        </section>
      ...
      {step === "analysis" && (
        <section className="screen center">
          ...
          <div className="analysis">
            ...
            {["Bentuk wajah", "Tekstur rambut", "Ketebalan rambut", "Garis rambut", "Proporsi wajah"].map(
              (item, index) => (
                <p key={item}>
                  <i>{progress > index * 20 ? "✓" : "·"}</i>
                  {item}
                </p>
              )
            )}
      ...
      {step === "results" && (
        <section className="screen">
            ...
            {styles.slice(0, 3).map((style, index) => (
              <article key={style[0]}>
                ...
                <div className="stylepic">
                  {photo ? <img src={photo} alt="" /> : <span>{["✦", "◒", "✂"][index]}</span>}
                </div>
                <div>
                  <small>{style[2]}</small>
                  <h3>{style[0]}</h3>
          ...
          <button className="outline wide" onClick={() => setStep("collection")}>
      ...
      {step === "collection" && (
        <section className="screen">
          ...
          <div className="filters">
            <button>Semua</button>
            <button>Pendek</button>
            <button>Medium</button>
            <button>Panjang</button>
          </div>

          <div className="grid">
            {collection.map((name, index) => (
              <button key={name} onClick={() => selectStyle(styles[index % styles.length])}>
                <span>{["◉", "◒", "◐", "◓"][index % 4]}</span>
                <b>{name}</b>
              </button>
            ))}
      ...
      {step === "preview" && (
        <section className="screen">
          ...
          <button className="cta wide" onClick={() => setStep("barber")}>
            ✂ Tunjukkan ke Barber
          </button>
      ...
      {step === "barber" && (
        <section className="screen">
          ...
          <h2>
            Siap dibawa <em>ke barber.</em>
          </h2>

          <div className="barber">
            <header>
              <div>
                <small>TOPSID RECOMMENDS</small>
                <h3>{selected[0]}</h3>
                <b>Cocok {selected[1]}%</b>
              </div>
              <strong>TOPSID</strong>
            </header>

            <div className="barberpic">
            ...
            <p>
              “Mas, saya mau model seperti ini. Bagian samping dibuat ringan,
              atas tetap natural.”
            </p>
          </div>
          ...
          <div className="actions">
            <button
              className="cta"
              onClick={() => alert("Demo: Barber Card siap dibagikan.")}
            >
              ↗ Bagikan
            </button>
      ...
      <footer className="foot">
        <b>TOPSID</b>
        <span>Cek dulu. Baru potong.</span>
        <span>© 2026</span>
      </footer>
    </main>
  );
